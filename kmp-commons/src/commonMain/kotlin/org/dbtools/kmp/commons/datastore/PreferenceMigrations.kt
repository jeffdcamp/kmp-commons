...
package org.dbtools.kmp.commons.datastore
...
class PreferenceMigrations(
    ...
    private fun findUpMigrationPath(
        ...
        while (if (upgrade) start < toVersion else start > toVersion) {
            val targetNodes: MutableMap<Int, PreferenceMigration> = migrationTree[start] ?: return emptyList()
            // keys are ordered so we can start searching from one end of them.
            val keySet: Set<Int> = if (upgrade) {
                targetNodes.keys.sortedBy { it }.toSet()
            } else {
                targetNodes.keys
            }
            var found = false
